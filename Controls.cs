...
public class AnimaNumUpDown : NumericUpDown
{
    private Color borderColor = Color.FromArgb(42, 42, 45);
    ...
    private Color buttonHighlightColor = Color.FromArgb(42, 42, 45);
    [DefaultValue(typeof(Color), "DarkGray")]
    ...
    private class UpDownButtonRenderer : NativeWindow
    {
        ...
        {
            if (m.Msg == 0xF /*WM_PAINT*/ && ((AnimaNumUpDown)updown.Parent).BorderStyle == BorderStyle.FixedSingle)
            {
                ...
                using (var g = Graphics.FromHdc(s.hdc))
                {
                    ...
                    using (var backBrush = new SolidBrush(enabled ? ((AnimaNumUpDown)updown.Parent).BackColor : SystemColors.Control))
                    {
                    ...
                    {
                        using (var b = new SolidBrush(((AnimaNumUpDown)updown.Parent).ButtonHighlightColor))
                        {
                            if (r1.Contains(p))
                        ...
                        using (var pen = new Pen(((AnimaNumUpDown)updown.Parent).BorderColor))
                        {
                            g.DrawLines(pen,
